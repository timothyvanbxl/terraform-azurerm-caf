...
resource "azurerm_mssql_virtual_network_rule" "network_rules" {
  ...
  name      = each.value.name
  server_id = azurerm_mssql_server.mssql.id
  subnet_id = try(each.value.subnet_id, var.vnets[try(var.client_config.landingzone_key, each.value.lz_key)][each.value.vnet_key].subnets[each.value.subnet_key].id, value.subnethardcoded)
}
