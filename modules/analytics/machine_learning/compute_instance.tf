module "compute_instance" {
  ...
  subnet_id                       = lookup(each.value, "lz_key", null) == null ? try(var.vnets[var.client_config.landingzone_key][each.value.vnet_key].subnets[each.value.subnet_key].id, each.value.subnethardcoded) : var.vnets[each.value.lz_key][each.value.vnet_key].subnets[each.value.subnet_key].id
  tags                            = local.tags
}
